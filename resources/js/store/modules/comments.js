...
export default {
    ...
    actions: {
        ...
        storeComment: (context, commentData) => {
            commentData.post(window.url + 'api/storeComment').then((response) => {
        ...
        updateComment: (context, commentData) => {
            commentData.post(window.url + 'api/updateComment/' + commentData.id).then((response) => {
                context.dispatch('getComments', {taskData: {id: commentData.task_id}})
                window.emitter.emit('resetCommentData')
                window.Toast.fire({
                    icon: 'success',
        ...
        deleteComment: (context, commentData) => {
            axios.post(window.url + 'api/deleteComment/' + commentData.id).then((response) => {
                context.dispatch('getComments', {taskData: {id: commentData.task_id}})
                window.Toast.fire({
                    icon: 'success',
